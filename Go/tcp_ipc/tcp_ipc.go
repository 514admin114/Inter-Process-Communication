...
// TCPIPC 使用TCP Socket进行进程间通信
type TCPIPC struct {
	address     string
	messageSize int
}

// NewTCPIPC 创建TCP IPC实例
func NewTCPIPC(messageSize int) *TCPIPC {
	return &TCPIPC{
...
func Producer(id int, address string, messageCount int, messageSize int, 
...
func RunTest(producers, consumers, messagesPerProducer, messageSize int) (*utils.PerformanceMetrics, error) {
	...
	listener, err := tcpIPC.StartServer()
	if err != nil {
		return nil, err
	}
	...
	for i := 0; i < producers; i++ {
		producerWg.Add(1)
		go Producer(i, tcpIPC.address, messagesPerProducer, messageSize, 
